  ...
  return (
    <div className="space-y-4 mb-6 sm:mb-8">
      ...
      {recent_games && recent_games.length > 0 && (
        <div className={`bg-${theme.cardBg} border border-${theme.accent}/30 rounded-lg p-4 sm:p-6 animate-slide-in transition-all duration-300`}>
          <h3 className={`text-base sm:text-lg font-bold text-${theme.accent} mb-4`}>Recent Games</h3>
              ...
              <tbody>
                {recent_games.slice(0, 5).map((game, index) => (
                  <tr key={index} className={`border-b border-${theme.accent}/10 hover:bg-${theme.accent}/5 transition-colors`}>
                    <td className={`py-2 pr-4 font-medium text-${theme.accent}`}>{game.score.toLocaleString()}</td>
                    <td className={`py-2 pr-4 text-${theme.text}`}>{game.accuracy_formatted}</td>
                    <td className={`py-2 pr-4 text-${theme.text}`}>{game.correct_answers}/{game.total_rounds}</td>
                    <td className={`py-2 pr-4 text-${theme.text} capitalize`}>{game.game_mode}</td>
                    <td className={`py-2 text-${theme.textMuted}`}>{new Date(game.achieved_at).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="sm:hidden space-y-2">
            {recent_games.slice(0, 5).map((game, index) => (
              <div key={index} className={`flex items-center justify-between py-2 border-b border-${theme.accent}/10 last:border-0`}>
                <div>
                  <span className={`font-bold text-${theme.accent} text-sm`}>{game.score.toLocaleString()} pts</span>
                  <div className={`text-xs text-${theme.textMuted} mt-0.5`}>
                    {game.correct_answers}/{game.total_rounds} · {game.accuracy_formatted} · <span className="capitalize">{game.game_mode}</span>
                  </div>
                </div>
                <span className={`text-xs text-${theme.textMuted}`}>{new Date(game.achieved_at).toLocaleDateString()}</span>
              </div>
            ))}
          </div>
        </div>
